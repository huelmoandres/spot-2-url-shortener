...
class DeleteUrlTest extends TestCase
{
    ...
    public function test_it_deletes_a_url_and_clears_cache_successfully(): void
    {
        $url = Url::factory()->create(['short_code' => 'aBcDeF']);

        // Simulate that the redirect cache exists
    ...
    public function test_it_returns_404_when_deleting_non_existent_short_code(): void
    {
        $response = $this->deleteJson('/api/urls/invalid1');

        $response->assertStatus(404)
            ->assertJson([
                'message' => 'Short code [invalid1] not found.',
            ]);
    }
}
